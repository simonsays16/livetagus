...
/** Garante "/admin-x" a partir de "admin-x", "/admin-x/" ou "admin-x/". */
const normalizeAdminRoute = (raw) => {
  const s = String(raw || "")
    .trim()
    .replace(/\/+$/, "");
  if (!s) return null;
...
module.exports = function registerRoutes(app, ctx) {
  ...
  const protectRoute = (req, res, next) => {
    ...
    if (!keyMatches(userKey, config.API_KEY)) {
      const htmlResponse = `
<!doctype html>
<html lang="pt-PT">
  <head>
  ...
  const adminAuth = (req, res, next) => {
    ...
    if (!keyMatches(userAdminKey, config.ADMIN_API_KEY)) {
  ...
  // check
  app.get(`${ADMIN_BASE}/ping`, adminAuth, (req, res) => {
    res.json({ ok: true, timestamp: new Date().toISOString() });
  });

  // vitais server
  app.get(`${ADMIN_BASE}/vitals`, adminAuth, (req, res) => {
    res.json({
      uptime: os.uptime(),
      freemem: os.freemem(),
  ...
  app.post(`${ADMIN_BASE}/avisos`, adminAuth, express.json(), (req, res) => {
    try {
  ...
  app.get(`${ADMIN_BASE}/pm2`, adminAuth, (req, res) => {
  ...
  app.post(
    ...
    (req, res) => {
      ...
      if (!procName || !/^[a-zA-Z0-9_\-]+$/.test(procName)) {
        return res
          .status(400)
          .json({ error: "Nome de processo inválido/crazy." });
      }
